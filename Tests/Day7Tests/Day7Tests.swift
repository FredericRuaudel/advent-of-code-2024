...
struct Day7Tests {
    ...
    @Test("Operator.allOperatorCombination should return all possible combinations of operators for given count")
    func operatorCombinationsTest() {
        let combinations1 = Operator.allOperatorCombination(of: 1)
        expectNoDifference(combinations1, Set([
            [.add],
            [.multiply],
        ]))

        let combinations2 = Operator.allOperatorCombination(of: 2)
        ...
        let combinations3 = Operator.allOperatorCombination(of: 3)
    ...
    func prepareOperatorCombinationsTest() throws {
        let equations = [
            try #require(CalibrationEquation(result: 42, operands: [12, 30])),
            try #require(CalibrationEquation(result: 100, operands: [20, 30, 50])),
            try #require(CalibrationEquation(result: 25, operands: [5, 10, 10]))
        ]
        
        let combinations = equations.prepareOperatorCombinations()
    ...
    func allCalibrationEquationParserTest() throws {
        ...
        let expectedEquations = [
            try #require(CalibrationEquation(result: 190, operands: [10, 19])),
            try #require(CalibrationEquation(result: 100, operands: [5, 10, 2])),
            try #require(CalibrationEquation(result: 42, operands: [6, 7]))
        ]
